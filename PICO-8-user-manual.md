...
| -export param_str    | 以无头模式运行 EXPORT 命令，随后退出（参阅导出部分下的[注解](#35-从宿主操作系统运行-export)） |
...
卡带运行的时候，每个被包含文件的内容就好像被粘贴到了编辑器里 `#INCLUDE` 那一行的位置一样。
...
- 把卡带当作数据文件，其会被加载到 PICO-8 编辑工具以进行修改  
...
- 精灵表的下半部分和地图的下半部分占据同一块内存。// 如果你不清楚这是怎么一回事，你最好只使用其中一部分  
...
关于控制代码和自定义字体的信息，参阅“[附录 A（P8SCII）](#71-附录-ap8scii-控制代码)”。
...
默认是颜色 0 作为透明色绘制（参阅 [PALT()](#paltc-t)）
...
如果指定了 `FILENAME`，数据就会直接写到磁盘上的那个卡带。一次会话内可以写入多达 64 个卡带。若要获取更多信息，参阅“[卡带数据](#611-卡带数据)”。
