...
如果省略了 `F` ，所有的标志就会被当作单一的位域进行获取 / 设置。
...
当 `COL` 设置了位 `0x1800.0000` ，并且 `PEEK(0x5F34) & 2 == 2` ，圆形会反转绘制。

### OVAL(X0, Y0, X1, Y1, [COL])

### OVALFILL(X0, Y0, X1, Y1, [COL])

绘制一个 x 和 y 上对称的椭圆形，按照给予的边界矩形。
...
绘制一个矩形或者实行矩形，对角的位置分别在（`X0`，`Y0`）和（`X1`，`Y1`）。
...
设置这一位的时候，填充图案会应用到精灵（`SPR`，`SSPR`，`MAP`，`TLINE`），使用到辅助调色板提供的颜色映射。
...
设置这一位的时候，辅助调色板也会应用到所有关注填充图案的绘图函数（`CIRCFILL`，`LINE` 等等）。与精灵绘图函数一同使用的时候比较有用，这样每个精灵像素的颜色索引和提供给绘图函数的颜色索引的含义是一样的。
...
填充图案也可以通过在任意的颜色参数中设置位来进行设置（例如，`COLOR()` 的参数，或者 `LINE()`、`RECT()` 的最后一个参数等等）。

`POKE(0x5F34, 0x3)` -- `0x1` 启用高位的填充图案，`0x2` 启用反转模式  
`CIRCFILL(64,64,20, 0x114E.ABCD)` -- 设置填充图案为 `ABCD`

使用颜色参数设置填充图案时，以下几位要用到：

位 `0x1000.0000` ：这需要设置，意思是“观测位 `0xf00.ffff`”  
位 `0x0100.0000` ：透明度  
位 `0x0200.0000` ：应用到精灵  
...
绘制从（`X0`，`Y0`）到（`X1`，`Y1`）的有纹理的直线，颜色值从地图采样。指定了 `LAYERS` 的时候，只有带有对应标志的精灵才能会被绘制（和 `MAP()` 相似）
